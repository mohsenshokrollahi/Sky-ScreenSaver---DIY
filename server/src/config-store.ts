...
export class ConfigStore {
  ...
  /** Fields manually patched by the user since the last displayMode switch. */
  private userOverriddenFields = new Set<string>();
  ...
  patch(patch: Partial<Config>): Config {
    ...
    const displayModeChanged =
      patch.displayMode !== undefined && patch.displayMode !== this.config.displayMode;

    // Track fields the user is manually patching (excluding displayMode itself).
    for (const key of Object.keys(patch)) {
      if (key !== "displayMode") {
        this.userOverriddenFields.add(key);
      }
    }
    ...
    if (displayModeChanged) {
      // Apply the new mode's preset, skipping any fields the user has
      // manually overridden since the last mode switch.
      this.config = applyDisplayModePreset(
        this.config,
        this.config.displayMode,
        this.userOverriddenFields,
      );
      // Clear the override set — fresh slate for the new mode.
      this.userOverriddenFields.clear();
    }
